...
unsigned int second = 10;   // 倒计时初始值改为10秒
...
void main(void)
{
    ...
    while(1)
    {
        ...
        switch(state)
        {
            case 0: if(second==0){second=3; state=1; SetLight();} break;  // 绿灯10秒→黄灯3秒
            case 1: if(second==0){second=10; state=2; SetLight();} break; // 黄灯3秒→南北绿灯10秒
            case 2: if(second==0){second=3; state=3; SetLight();} break;  // 绿灯10秒→黄灯3秒
            case 3: if(second==0){second=10; state=0; SetLight();} break; // 黄灯3秒→东西绿灯10秒
        }
    }
}
